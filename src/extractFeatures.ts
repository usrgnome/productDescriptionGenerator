...
export function extractFeatures(features: Array<string>, data: any){
      if (validateBoolean(data["product.features.requiredTransport.canFitInMotorbike"]))
      ...
      if (
        validateBoolean(data["product.features.productSpecification.chair.isStackable"])
      )
        features.push("is stackable");
      if (
        validateBoolean(data["product.features.productSpecification.chair.isFoldable"])
      )
